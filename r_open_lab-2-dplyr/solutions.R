...
# The mean(x) gives us the mean for the a group, and for the b group. The pipeline first sets up the groups of y (a and b), and then the summarize
# runs on the grouped tibble.
...
# We are summarizing, so the z grouping will get dropped. We get a group for each level of y with a different level of z, so a, K and a,L, then for
# b both values have K for z, so they will stay grouped together.
...
# Mutate will take the mean on the unique combinations of y and z. So there will be one mean for row 1 (a and K), one for rows 2 and 5 (b and K),
# and one for 3 and 4 (a and L). The mean for the "b and K" rows and the "a and L" rows are calculated differently:
# a and L: (3+4+/2)=3.5
# b an K: (2+5)/2=3.5
